 ...
 *   VERTEX_REASONING_ENGINE_ID  – full Reasoning Engine resource name (required; no fallback in code)
...
/** SecOps G.O.A.T.: sem nome de recurso GCP hardcoded — só via ambiente. */
...
export class VertexReasoningClient {
  ...
  async init() {
    if (!REASONING_ENGINE_RESOURCE) {
      throw new Error(
        'VERTEX_REASONING_ENGINE_ID ausente — defina o nome completo do Reasoning Engine (deploy do Líder Supremo agent_1777236402725). Nenhum fallback em código.',
      );
    }
