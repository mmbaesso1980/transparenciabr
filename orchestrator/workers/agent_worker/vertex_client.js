 ...
 *   VERTEX_REASONING_ENGINE_ID  – full resource name (obrigatório; sem fallback hardcoded)
...
/** Definido em init() — obrigatório via env (G.O.A.T. Pilar 3: sem resource GCP hardcoded). */
let reasoningEngineResource = '';
...
export class VertexReasoningClient {
  ...
  async invokeAgent(agentId, prompt, tools = []) {
    ...
    const request = {
      reasoningEngine: reasoningEngineResource,
      input: {
        input: prompt,
