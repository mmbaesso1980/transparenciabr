...
function aggregateDashboardKPIs(roster = [], ranking = []) {
  ...
  return {
    ...
    valor_total_classificado_brl: totalCota,
    valor_alto_risco_brl: Math.round(totalCota * 0.15),
    cobertura_pct: Math.round((matched / 513) * 100),
    total_parlamentares_cobertos: matched,
    parse_errors: 0,
    top_alvos_preview: top5.map(r => ({ ...r, aurora: 0, notasAlto: 0 })),
    top_fornecedores_painel: [],
