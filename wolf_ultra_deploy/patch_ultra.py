...
def patch_runner():
    ...
    method = (
        ...
        f"{indent}        # reusa o engine do autostart (idempotente) -> UM cerebro so.\n"
        f"{indent}        self._ultra_engine = arm_autostart(self.client)\n"
        ...
        f"{indent}        try:\n"
        f"{indent}            from wolf_trader.ultra import disarm_autostart as _disarm\n"
        f"{indent}            _disarm()  # impede o scheduler de reiniciar jogos apos parada manual\n"
        f"{indent}        except Exception: pass\n"
        f"{indent}        ue.stop()\n"
        f"{indent}        _ultra_tg('🛑 /stopwolfultra — ultra encerrado, posições recolhidas (exceto Renan-YES). Scheduler desarmado.'); return\n"
        f"{indent}{END}\n"
    )
