...
def init_firestore() -> firestore.Client:
    """Firestore com emulador local ou credenciais GCP."""
    try:
        if not firebase_admin._apps:
            ...
            project_id = (
                ...
                or "transparenciabr-local"
            )
            if not emulator_host:
                emulator_host = "127.0.0.1:8080"
                os.environ["FIRESTORE_EMULATOR_HOST"] = emulator_host
                logger.warning(
                    "FIRESTORE_EMULATOR_HOST ausente; usando fallback local %s.",
                    emulator_host,
                )

            mock_key_file = Path(__file__).resolve().parent / "mock_key.json"
            if emulator_host and mock_key_file.is_file():
                os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = str(mock_key_file)

            if emulator_host:
                logger.info(
                    "FIRESTORE_EMULATOR_HOST=%s — projectId=%s.",
                    emulator_host,
                    project_id,
                )
        ...
        logger.exception("Falha ao inicializar Firebase Admin / Firestore: %s", exc)
        raise
