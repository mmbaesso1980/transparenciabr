    ...
    while True:
        ...
        for tentativa in range(3):
            try:
                ...
                logger.warning("Falha na API Transparência (orgao=%s, pagina=%d, tentativa %d): %s", orgao_id, pagina, tentativa + 1, e)
                time.sleep(2)

        if not sucesso:
            logger.error("Falha ao buscar servidores para o órgão %s. Retornando os resultados parciais.", orgao_id)
            break

    return resultados

def main() -> int:
    parser = argparse.ArgumentParser(description="F.L.A.V.I.O. — Caçador de Fantasmas via Portal da Transparência.")
    ...
    for doc in politicos_docs:
        ...
        # O órgão da câmara dos deputados é 20101 no portal da transparência.
        ...
        for serv in servidores:
            ...
            # Idempotência obrigatória com hash SHA-256
            ...
            db.collection(COLLECTION_GHOST_SERVIDORES).document(doc_id).set(doc_body, merge=True)
            escritos += 1
