...
export default function NepotismoCruzadoPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setLoading(true);
        const res = await fetch("/api/datalake/nepotismo-cruzado-kpis");
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm text-muted-foreground">Carregando análise de nepotismo cruzado...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-background">
      <div className="border-b border-border bg-card/50">
        <div className="container mx-auto px-4 py-6">
          ...
          <h1 className="text-3xl font-bold text-foreground">Nepotismo Cruzado</h1>
          <p className="text-sm text-muted-foreground mt-2">
            {data?.mensagem || "Pares de parlamentares que compartilham fornecedores"}
          </p>
        </div>
      </div>
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
              <div className="p-4 bg-card border border-border rounded-lg">
                ...
                <p className="text-xs text-muted-foreground mt-2">Fornecedores distintos na base CEAP</p>
              </div>

              <div className="p-4 bg-card border border-border rounded-lg">
                <div className="flex items-center gap-2 mb-2">
                  <Users size={16} className="text-purple-500" />
                  <p className="text-sm font-medium text-muted-foreground">Total Parlamentares</p>
                </div>
                <div className="text-3xl font-bold text-foreground">{fmt(r.total_parlamentares)}</div>
                <p className="text-xs text-muted-foreground mt-2">Parlamentares analisados</p>
              </div>
            </div>

            {data.paresCruzados?.length > 0 && (
              <div className="mb-8">
                <h2 className="text-xl font-bold text-foreground mb-4">Pares com Mais Fornecedores em Comum (15+)</h2>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-border bg-red-500/5">
                          ...
                          <th className="text-left p-3 font-medium text-red-600">Parlamentar A</th>
                          <th className="text-left p-3 font-medium text-red-600">Parlamentar B</th>
                          <th className="text-right p-3 font-medium text-red-600">Fornecedores em Comum</th>
                        </tr>
                      </thead>
                      <tbody>
                        {data.paresCruzados.map((p, i) => (
                          <tr key={i} className="border-b border-border/50 hover:bg-red-500/5">
                            <td className="p-3 text-muted-foreground">{i + 1}</td>
                            <td className="p-3 font-medium text-foreground">{p.parlamentar_a}</td>
                            <td className="p-3 font-medium text-foreground">{p.parlamentar_b}</td>
                            <td className="p-3 text-right text-red-600 font-bold">{fmt(p.fornecedores_em_comum)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
